...
+ (FlxReferencedExecution *) execWithCount:(NSUInteger)count block:(void (^)(void))block{
    FlxReferencedExecution *exec = [[FlxReferencedExecution alloc] initWithBlock:block];
...
- (void) execute{
    _count = 0;
    for (VoidBlock block in _blocks) block();
