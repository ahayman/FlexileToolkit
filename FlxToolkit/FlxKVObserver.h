...
@property (nonatomic, readonly) SEL observedKey;
@end
...
/**
 ...
 Only a single observation can be created for each object/key pair from a single observer.  If the observer attempts to create an observation with a previous object/key pair, that previous pair will be replaced, regardless of whether it is a target/action or block style observation.
...
/**
 Action/Target style Key Value Observation. Observations are attached to the observer, which means when the observer is dealloc'd the observation will also be removed. This method will immediately fire an observation.
 Only a single observation can be created for each object/key pair from a single observer.  If the observer attempts to create an observation with a previous object/key pair, that previous pair will be replaced, regardless of whether it is a target/action or block style observation.
 It's important to note that the observation retains the observed object and the observation is attached to the observer. This means if the observer will retain the observed object until it stops observing it (if it's replaced or explicitly stopped).
 @param SEL key - The key to be observed
 @param id object - The object on which the key is to be observed. The observed is retained.
...
/**
 Block style Observation.  Observations are attached to the observed object. When the observed object is dealloc'd, the observation will also be removed. This will call the block with the initial value when the object is first observed.
 Only a single observation can be created for each object/key pair from a single observer.  If the observer attempts to create an observation with a previous object/key pair, that previous pair will be replaced.
 It's important to note that the observation retains the observed object and the observation is attached to the observer. This means if the observer will retain the observed object until it stops observing it (if it's replaced or explicitly stopped).
 @param SEL key - The key to be observed
 @param id object - The object on which the key is to be observed
...
/**
 Remove observation of object with the specified key.
...
/**
 Remove all observations for an observer observing specified object
